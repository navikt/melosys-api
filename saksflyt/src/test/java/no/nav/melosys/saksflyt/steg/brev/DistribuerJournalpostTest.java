...
class DistribuerJournalpostTest {
    ...
    void utførDistribuerJournalpostUtenAdresse() {
        ...
        Prosessinstans prosessinstans = setupHappypath(journalpostId, Aktoersroller.BRUKER, Distribusjonstyper.VIKTIG);

        distribuerJournalpost.utfør(prosessinstans);

        verify(mockDoksysFasade).distribuerJournalpost(eq(journalpostId), eq(Distribusjonstyper.VIKTIG));
    }
    ...
    void utførDistribuerJournalpostMedPostadresse() {
        ...
        Prosessinstans prosessinstans = setupHappypath(journalpostId, Aktoersroller.REPRESENTANT, Distribusjonstyper.ANNET);
        ...
        verify(mockDoksysFasade).distribuerJournalpost(eq(journalpostId), any(StrukturertAdresse.class), any(), any(), eq(Distribusjonstyper.ANNET));
    }

    @Test
    void utførDistribuerJournalpostMedForretningsadresse() {
        String journalpostId = "12345";
        Prosessinstans prosessinstans = setupHappypath(journalpostId, Aktoersroller.REPRESENTANT, Distribusjonstyper.VEDTAK);
        prosessinstans.setData(ProsessDataKey.ORGNR, "123456789");
        ...
        verify(mockDoksysFasade).distribuerJournalpost(eq(journalpostId), any(StrukturertAdresse.class), any(), any(), eq(Distribusjonstyper.VEDTAK));
    }
    ...
    void utførDistribuerJournalpostMedReperesentantPerson() {
        ...
        verify(mockDoksysFasade).distribuerJournalpost(eq(journalpostId), eq(Distribusjonstyper.ANNET));
    }
    ...
    void utførDistribuerJournalpostMedUtenlandskMyndighet() {
        ...
        Prosessinstans prosessinstans = setupHappypath(journalpostId, Aktoersroller.TRYGDEMYNDIGHET, Distribusjonstyper.VIKTIG);
        ...
        verify(mockDoksysFasade).distribuerJournalpost(eq(journalpostId), any(StrukturertAdresse.class), eq(Distribusjonstyper.VIKTIG));
    }

    private Prosessinstans setupHappypath(String journalpostId, Aktoersroller rolle, Distribusjonstyper distribusjonstyper) {
        Behandling behandling = TestdataFactory.lagBehandling();
        Prosessinstans prosessinstans = new Prosessinstans();
        DokgenBrevbestilling brevbestilling = new MangelbrevBrevbestilling.Builder()
            .medDistribusjonstype(distribusjonstyper)
            .build();
