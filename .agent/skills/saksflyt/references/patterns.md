...
For detailed race condition investigation, see **[stale-reference-problem.md](stale-reference-problem.md)**.

### Quick Symptom Guide

| Symptom | Likely Cause | Investigation |
|---------|--------------|---------------|
| `OptimisticLockingException` | Sync listener + saga conflict | Check for BEFORE_COMMIT listeners |
| Silent data corruption (field reverts) | Stale reference or cross-TX loading | Add debug logging to trace |
| `StaleObjectStateException` | Detached entity flushed | Check entity loading patterns |
| Sporadic failures (works sometimes) | Timing-dependent race | Run E2E tests repeatedly |
...
2. **Check for synchronous listeners on same status**
   ```bash
   grep -r "BehandlingEndretStatusEvent\|@EventListener" --include="*.java" --include="*.kt" \
     | grep -v "AFTER_COMMIT"
   ```

3. **Check for cross-behandling entity loading in saga steps**
   ```bash
   grep -r "fagsak.behandlinger\|hentBehandlingsresultat" --include="*.java" --include="*.kt" \
     saksflyt/src/main/
   ```

4. **Add targeted debug logging**
   ```kotlin
   // In VedtakService before saga
   log.info("RACE_DEBUG: Before saga - entityField=${entity.field}, version=${entity.version}")

   // In saga step
   log.info("RACE_DEBUG: Saga sees - entityField=${entity.field}, version=${entity.version}")
   ```

5. **Run E2E tests repeatedly**
   ```bash
   for i in {1..10}; do
     ./gradlew :melosys-e2e-tests:test --tests "SpecificTest" && echo "Pass $i" || echo "FAIL $i"
   done
   ```

6. **Download and analyze logs from CI**
   ```bash
   gh run download <run-id> -n test-summary -D /tmp/artifacts
   grep -r "RACE_DEBUG\|OptimisticLock" /tmp/artifacts/
   ```

### Proven Fixes

| Problem | Fix | Example PR |
|---------|-----|------------|
| Sync listener conflicts | Move to saga step | PR #3161 (LAGRE_PERSONOPPLYSNINGER) |
| Cross-TX entity overwrites | @DynamicUpdate | PR #3160 (Behandlingsresultat) |
| Stale entity in service | Reload before modify | PR #3166 (OppgaveService) |
| equals/hashCode with @Lob | Use id-based equals | See saksopplysningkilde-equals-hashcode-fix.md |

### Defense in Depth Strategy

When fixing race conditions, apply multiple layers:

1. **Root cause fix** (e.g., reload Prosessinstans in AFTER_COMMIT)
2. **@DynamicUpdate** on entities modified by multiple transactions
3. **saveAndFlush()** before saga start for explicit ordering
4. **Move sync listeners to saga steps** for correct architecture
