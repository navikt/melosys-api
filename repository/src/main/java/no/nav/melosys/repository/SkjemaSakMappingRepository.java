...
public interface SkjemaSakMappingRepository extends JpaRepository<SkjemaSakMapping, UUID> {
     ...
     *
     * <p>Claim-rader ekskluderes ({@code originalData is null}). Det er reservasjoner skrevet av en
     * relatert innsending for å binde et skjema til saken før skjemaet selv er mottatt
     * (MELOSYS-8151). De representerer ingen mottatt innsending, og skal derfor ikke rapportere
     * saksstatus tilbake til skjema-api. Raden får originalData når innsendingen faktisk kommer.
     *
    ...
    String SAKSSTATUS_SYNK_PROJEKSJON =
        "select m.skjemaId as skjemaId, f.saksnummer as saksnummer, f.status as saksstatus "
            + "from SkjemaSakMapping m join m.fagsak f where m.originalData is not null";
    ...
    @Query(SAKSSTATUS_SYNK_PROJEKSJON + " and f.saksnummer = :saksnummer")
