...
if [ $USE_INTEGRATION_TEST -eq 1 ]; then
  # For integration tests, check if -am is used
  if echo "$CMD_ARGS" | grep -q -- "-am"; then
    # With -am, use verify to compile dependencies, skip surefire, run failsafe
    MVN_CMD="verify -Dsurefire.skip=true"
  else
    # Without -am, use direct goal (faster)
...
# Check if tests actually passed (no failures, no errors, and Maven succeeded)
# Note: failsafe:integration-test doesn't fail the build, so we need to check the summary
if [ $RESULT -eq 0 ] && [ "${FAILURES:-0}" -eq 0 ] && [ "${ERRORS:-0}" -eq 0 ]; then
  echo "✅ SUCCESS"
  echo "$SUMMARY"
