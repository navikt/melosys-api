...
public class SaksbehandlingstidSoknad extends DokgenDto {
    ...
    private final Mottakerroller avsenderTypeSoknad;
    ...
    public Mottakerroller getAvsenderTypeSoknad() {
    ...
    private Mottakerroller utledAvsendertype(Avsendertyper avsendertype) {
        if (avsendertype == null) {
            return BRUKER;
        }
        return switch (avsendertype) {
            case PERSON -> BRUKER;
            case ORGANISASJON -> FULLMEKTIG;
            case UTENLANDSK_TRYGDEMYNDIGHET -> UTENLANDSK_TRYGDEMYNDIGHET;
        };
    }
}
