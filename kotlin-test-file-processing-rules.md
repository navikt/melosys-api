...
```
Also stive for using expression body so instead of using variables we can often inline
```kotlin
val saksopplysning = Saksopplysning().apply {
...
// Also CORRECT - when you need a fagsak with 1 or more behandling:
val fagsak = Fagsak.forTest {
    medBruker()
    behandling {
        type = Behandlingstyper.FØRSTEGANG
...
// And see if you can instead of using variables we can inline objects.
val behandling = Behandling.forTest {
    id = 543
