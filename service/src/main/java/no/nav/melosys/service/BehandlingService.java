...
public class BehandlingService {
    ...
    public boolean aktivProsessinstansEksistererFor(long behandlingsid) {
        ...
        if (aktivProsessinstans.isPresent()) {
            log.debug(String.format("BehandlingsId %d%n har aktiv prosessinstans", behandlingsid));
            return true;
        }
        log.debug(String.format("BehandlingsId %d%n har ingen aktiv prosessinstans", behandlingsid));
        return false;
    }
}
