...
public final class OppgaveFactory {
    ...
    public static Oppgave.Builder lagBehandlingsOppgaveForType(Behandlingstema behandlingstema, Behandlingstyper behandlingstype, LocalDate mottaksdato) {
        OppgaveParametere parametere = hentOppgaveParametere(behandlingstema, mottaksdato);
    ...
    private static OppgaveParametere hentOppgaveParametere(Behandlingstema behandlingstema, LocalDate mottaksdato) {
        ...
        switch (behandlingstema) {
            ...
            case IKKE_YRKESAKTIV:
                oppgaveParametere = new OppgaveParametere(Tema.MED, Oppgavetyper.BEH_SAK_MK, fristDager(mottaksdato, 30));
                break;
            case REGISTRERING_UNNTAK_NORSK_TRYGD_UTSTASJONERING:
            case REGISTRERING_UNNTAK_NORSK_TRYGD_ØVRIGE:
                oppgaveParametere = new OppgaveParametere(Tema.UFM, Oppgavetyper.BEH_SED, fristUker(mottaksdato, 2));
                break;
    ...
    private static LocalDate fristUker(LocalDate mottaksdato, int uker) {
        return mottaksdato == null ? fristUker(uker) : mottaksdato.plusWeeks(uker);
    }
    ...
    private static LocalDate fristDager(LocalDate mottaksdato, int dager) {
        return mottaksdato == null ? fristDager(dager) : mottaksdato.plusDays(dager);
    }
