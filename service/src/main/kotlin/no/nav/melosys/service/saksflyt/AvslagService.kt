...
open class AvslagService(
    ...
    @Transactional
    open fun avslåPgaManglendeOpplysninger(behandlingID: Long, fritekst: String, bestillersId: String) {
        val behandling = behandlingService.hentBehandling(behandlingID)
        val behandlingsresultat = behandlingsresultatService.hentBehandlingsresultat(behandlingID)
        behandlingsresultat.apply {
            type = Behandlingsresultattyper.AVSLAG_MANGLENDE_OPPL
            fastsattAvLand = Land_iso2.NO
            this.settVedtakMetadata(Vedtakstyper.FØRSTEGANGSVEDTAK, LocalDate.now().plusWeeks(FRIST_KLAGE_UKER))
        }
