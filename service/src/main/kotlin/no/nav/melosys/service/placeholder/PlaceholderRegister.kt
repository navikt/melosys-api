...
class PlaceholderRegister {
    ...
    val definisjoner: List<PlaceholderDefinisjon> = listOf(
        ...
        PlaceholderDefinisjon(
            ...
            resolver = { kontekst ->
                dato(
                    kontekst.sakskontekst.medlemskapsperiodeFom,
                    kontekst.sakskontekst.innvilgedePerioder().mapNotNull { it.fom },
                )
            },
        ),
        PlaceholderDefinisjon(
            ...
            resolver = { kontekst ->
                dato(
                    kontekst.sakskontekst.medlemskapsperiodeTom,
                    kontekst.sakskontekst.innvilgedePerioder().mapNotNull { it.tom },
                )
            },
        ),
        PlaceholderDefinisjon(
            ...
            beskrivelse = "Startdatoen for perioden det er søkt om",
            eksempel = { "01.03.2024" },
            resolver = { kontekst -> datoer(kontekst.sakskontekst.soknadsperioder.mapNotNull { it.fom }) },
        ),
        PlaceholderDefinisjon(
            nokkel = "soknadsperiode-til",
            visningsnavn = "Søknadsperiode til",
            beskrivelse = "Sluttdatoen for perioden det er søkt om. Utelates ved åpen sluttdato",
            eksempel = { "28.02.2027" },
            resolver = { kontekst -> datoer(kontekst.sakskontekst.soknadsperioder.mapNotNull { it.tom }) },
        ),
        PlaceholderDefinisjon(
        ...
        PlaceholderDefinisjon(
            ...
            eksempel = { "0155" },
            resolver = { verdi(it.bostedsadresse()?.postnummer) },
        ),
        PlaceholderDefinisjon(
            ...
            eksempel = { "Oslo" },
            resolver = { verdi(it.bostedsadresse()?.poststed) },
        ),
        PlaceholderDefinisjon(
            ...
            resolver = { kontekst ->
                adresser(kontekst.persondata.oppholdsadresseKandidater().map { it.strukturertAdresse }, kontekst.landnavn)
            },
        ),
        PlaceholderDefinisjon(
            ...
            resolver = { kontekst ->
                adresser(
                    kontekst.persondata.kontaktadresseKandidater().map { it.hentEllerLagStrukturertAdresse() },
                    kontekst.landnavn,
                )
            },
        ),
    ...
    private companion object {
        ...
        private fun dato(dato: LocalDate?, kandidater: List<LocalDate> = emptyList()): PlaceholderResultat? =
            dato?.let { valgt -> PlaceholderResultat(valgt.format(DATOFORMAT), kandidater.map { it.format(DATOFORMAT) }) }

        /** Første element er forhåndsvalget – resten er alternativer i rekkefølgen kilden ga dem. */
        private fun datoer(datoer: List<LocalDate>): PlaceholderResultat? = dato(datoer.firstOrNull(), datoer)

        private fun tekster(tekster: List<String>): PlaceholderResultat? =
            tekster.firstOrNull()?.let { PlaceholderResultat(it, tekster) }

        private fun adresser(adresser: List<StrukturertAdresse?>, landnavn: (String?) -> String): PlaceholderResultat? =
            tekster(adresser.filterNotNull().map { it.tilAdresselinje(landnavn) })

        private fun PlaceholderKontekst.bostedsadresse(): StrukturertAdresse? =
            persondata.finnBostedsadresse().getOrNull()?.strukturertAdresse
    }
}
