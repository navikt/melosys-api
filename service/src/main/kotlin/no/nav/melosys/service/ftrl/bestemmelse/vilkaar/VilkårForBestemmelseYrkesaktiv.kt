...
class VilkårForBestemmelseYrkesaktiv(val mottatteOpplysningerService: MottatteOpplysningerService) {
    fun hentVilkår(
        ...
        return when (bestemmelse) {
            ...
            ARKTISK_RÅDS_SEKRETARIAT_ART16 -> listOf(
                Vilkår(ARKTISK_RÅDS_SEKRETARIAT_ART16_STABSMEDLEM)
            )

            DET_INTERNASJONALE_BARENTSSEKRETARIATET_ART14 -> listOf(
                Vilkår(DET_INTERNASJONALE_BARENTSSEKRETARIATET_ART14_FAST_STAB)
            )

            DEN_NORDATLANTISKE_SJØPATTEDYRKOMMISJON_ART16 -> listOf(
                Vilkår(DEN_NORDATLANTISKE_SJØPATTEDYRKOMMISJON_ART16_TJENESTEMANN)
            )

            TILLEGGSAVTALE_NATO -> listOf(
                Vilkår(TILLEGGSAVTALE_NATO_SIVILT_ANSATT)
            )
