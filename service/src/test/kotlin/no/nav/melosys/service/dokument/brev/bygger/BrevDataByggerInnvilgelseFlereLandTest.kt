...
class BrevDataByggerInnvilgelseFlereLandTest {
    ...
    fun setUp() {
        ...
        every { lovvalgsperiodeService.hentLovvalgsperiode(any()) } returns lovvalgsperiodeForTest()
    ...
    private fun lagBehandling(
        tema: Behandlingstema = Behandlingstema.REGISTRERING_UNNTAK_NORSK_TRYGD_UTSTASJONERING
    ) = Behandling.forTest {
        id = 1L
        this.tema = tema
        fagsak { medBruker() }
    ...
    private fun lagBrevressurser(behandling: Behandling = lagBehandling()): BrevDataGrunnlag =
        BrevDataGrunnlag(
    ...
    fun `lag utpeking annet land skal sette trydemyndighetsland`() {
        ...
        val brevdataressurser = lagBrevressurser(behandling)
        val brevData = brevDataByggerInnvilgelse.lag(brevdataressurser, saksbehandler) as BrevDataInnvilgelseFlereLand
