...
class FtrlVedtakServiceTest {
    ...
    fun fattVedtak_delvis_opphørt_fatterVedtak() {
        ...
        verify { oppgaveService.ferdigstillOppgaveMedSaksnummer(FagsakTestFactory.SAKSNUMMER) }
        verify { dokgenService.produserOgDistribuerBrev(BEH_ID, capture(brevbestillingRequestSlot)) }
        verify(exactly = 0) { vilkaarsresultatService.tilbakestillVilkårsresultatFraBehandlingsresultatOgLagre(any()) }
    ...
    fun fattVedtak_opphørt_fatterVedtak() {
        ...
        verify { oppgaveService.ferdigstillOppgaveMedSaksnummer(FagsakTestFactory.SAKSNUMMER) }
        verify { dokgenService.produserOgDistribuerBrev(BEH_ID, capture(brevbestillingRequestSlot)) }
        verify { vilkaarsresultatService.tilbakestillVilkårsresultatFraBehandlingsresultatOgLagre(BEH_ID) }
