...
class DokgenServiceKtTest {
    ...
    @BeforeEach
    fun init() {
        val dokgenMapperDatahenter = mockk<DokgenMapperDatahenter>(relaxed = true)

        dokgenService = DokgenService(
            mockDokgenConsumer, DokumentproduksjonsInfoMapper(), mockJoarkFasade,
            DokgenMalMapper(
                dokgenMapperDatahenter, mockInnvilgelseFtrlMapper, mockInnvilgelseEftaStorbritanniaMapper,
    ...
    fun `produserBrev feiler utilgjengelig mal`() {
        ...
        every { mockBehandlingsService.hentBehandlingMedSaksopplysninger(123L) } returns lagBehandling()

        shouldThrow<FunksjonellException> {
            dokgenService.produserBrev(Mottaker(), brevbestilling)
        }.message shouldBe "ProduserbartDokument ATTEST_A1 er ikke støttet"
    }
    ...
    fun `produserBrev til bruker ok`() {
        ...
        every { mockUtledMottaksdato.getMottaksdato(any(), isNull()) } returns LocalDate.now()
    ...
    @Test
    fun `er tilgjengelig dokgen mal`() {
        dokgenService.erTilgjengeligDokgenmal(MELDING_FORVENTET_SAKSBEHANDLINGSTID_SOKNAD).shouldBeTrue()
        dokgenService.erTilgjengeligDokgenmal(ATTEST_A1).shouldBeFalse()
    }
