...
/**
 * Container-based equivalent of SatsendringTestBase.
 *
 * Uses:
 * - Melosys-mock container for journalføring
 * - WireMock (in-process) for trygdeavgiftsberegning API with satsendring transformer
 */
abstract class ContainerSatsendringTestBase : ContainerJournalfoeringBase(TrygdeavgiftsberegningMedSatsendring()) {

    @Autowired
    protected lateinit var avklartefaktaService: AvklartefaktaService

    @Autowired
    protected lateinit var mottatteOpplysningerService: MottatteOpplysningerService

    @Autowired
    protected lateinit var opprettForslagMedlemskapsperiodeService: OpprettForslagMedlemskapsperiodeService

    @Autowired
    protected lateinit var trygdeavgiftsberegningService: TrygdeavgiftsberegningService

    @Autowired
    protected lateinit var vedtaksfattingFasade: VedtaksfattingFasade

    @Autowired
    protected lateinit var vilkaarsresultatService: VilkaarsresultatService

    protected var originalSubjectHandler: SubjectHandler? = null

    @BeforeEach
    fun setupSatsendringBase() {
